...
then
    ...
    mkdir -p services/$SERVICE_UNDERSCORE_NAME/db/migrations
    mkdir -p services/$SERVICE_UNDERSCORE_NAME/db/query
    mkdir -p services/$SERVICE_UNDERSCORE_NAME/db/sqlc
    mkdir -p services/$SERVICE_UNDERSCORE_NAME/proto
    ...
    envsubst '\${SERVICE_UNDERSCORE_NAME} \${SERVICE_PASCALCASE_NAME}' < .scaffold/template/service/Dockerfile.template > services/$SERVICE_UNDERSCORE_NAME/Dockerfile
